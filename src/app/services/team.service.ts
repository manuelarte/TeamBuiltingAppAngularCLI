...
import {Timeslice} from "app/timeslice";
...
export class TeamService {
  ...
  constructor(protected httpClient: HttpClient, private authHttp: AuthHttp) { }
  ...
  postTeam(team: Team): Promise<Team> {
    return this.postTeam$(team).toPromise();
  }
  ...
  getPlayers(id: string, date: Date = null): Promise<PlayerToTeam[]> {
    ...
    return this.httpClient.get<PlayerToTeam[]>(`${this.playerToTeamsUrl}/teams/${id}`, {params: params})
      .toPromise();
  }
  ...
  private convertFromDateAndToDateGeneric<T extends Timeslice>(value: T): T {
    value.fromDate = new Date(value.fromDate);
    value.toDate == null ? null : value.toDate = new Date(value.toDate);
    return value;
  }
